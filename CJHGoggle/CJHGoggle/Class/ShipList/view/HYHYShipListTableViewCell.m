...
- (void)setShipListModel:(HYShipListModel *)shipListModel
{
    ...
    self.stationLab.text = [NSString stringWithFormat:@"近期靠泊：%@ %@", shipListModel.sitename?:@"", shipListModel.utc];
    
    NSInteger cjhState = shipListModel.cjhState.integerValue;
    ...
    if(cjhState == 2) {
        ...
        if(course >= 0 && course <= 180) {
            ...
            self.imgView.image = [UIImage imageNamed:@"cjh_v_moor_right"];
        }
        else if(course > 180 && course <= 360) {
            
            self.imgView.image = [UIImage imageNamed:@"cjh_v_moor_left"];
        }
    }
    else {
