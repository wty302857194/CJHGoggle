...
#pragma mark - 删除全部历史记录

- (void)emptySearchHistoryDidClick
{
    [self.searchHistories removeAllObjects];
    self.delegate ? [self.delegate didAllDeleteSearchViewController:self] : nil;
...
- (void)closeDidClick:(UIButton *)sender
{
    ...
    NSIndexPath *index = [self.baseSearchTableView indexPathForCell:cell];
    
    self.delegate ? [self.delegate searchViewController:self didDeleteSearchSuggestionAtIndexPath:index searchText:cell.textLabel.text] :nil;
