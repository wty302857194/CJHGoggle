...
- (void)fetchShipDetailInfo {
    ...
    [HYHttpManager postRequestWithUrl:url params:params parserClass:[HYShipDetailInfo class] successBlock:^(id response) {
        ...
        if (detail) {
            ...
            if (weakSelf.detailInfo.ismyship && weakSelf.detailInfo.labelList && weakSelf.detailInfo.labelList.count < 5) {
                //添加地图和搜索按钮
...
- (void)loadDataSource {
    ...
    if ([self.shipInfoHeaderView currentSelectedType] == HYSHIPINFO_HEAD_USER) {
        ...
        if (!self.detailInfo.oilRecordList) return;
        
        if (!self.detailInfo.nonOilRecords) return;
