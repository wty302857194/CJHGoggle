...
@implementation HYCustomOverlay
+ (HYCustomOverlay *)customWithPoints:(BMKMapPoint *)points count:(NSUInteger)count
{
	HYCustomOverlay* polyline = [[HYCustomOverlay alloc]initWithPoints:points count:count];
	return polyline;
}
...
-(BMKMapRect)boundingMapRect {
		...
		if (_points != nil && _pointCount > 0) {
			...
			for (int i = 0; i < _pointCount; i++) {
				_boundingMapRect = [HYCustomOverlay MapRectUnionWithPoint:_boundingMapRect point:_points[i]];
			}
		}
